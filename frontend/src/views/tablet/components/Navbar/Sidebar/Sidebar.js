...
const Sidebar = (props) => {
    ...
    const isTouching = () => {
        return touch.touchIdentifier !== null;
    }
    ...
    const inCancelDistanceOnScroll = () => {
        ...
        if (props.pullRight) {
          cancelDistanceOnScroll =
            Math.abs(touch.touchCurrentX - touch.touchStartX) <
            CANCEL_DISTANCE_ON_SCROLL;
        } else {
          cancelDistanceOnScroll =
            Math.abs(touch.touchStartX - touch.touchCurrentX) <
            CANCEL_DISTANCE_ON_SCROLL;
        }
    ...
    const touchSidebarWidth = () => {
        ...
        if (props.pullRight) {
          if (
            props.open &&
            innerWidth - touch.touchStartX < sidebarWidth
          ) {
            if (touch.touchCurrentX > touch.touchStartX) {
              return (
                touch.sidebarWidth +
                touch.touchStartX -
                touch.touchCurrentX
          ...
          return Math.min(
            innerWidth - touch.touchCurrentX,
            sidebarWidth
          );
        }
    
        if (props.open && touch.touchStartX < sidebarWidth) {
          if (touch.touchCurrentX > touch.touchStartX) {
            return sidebarWidth;
          }
          return (
        ...
        return Math.min(touch.touchCurrentX, sidebarWidth);
      }
    ...
    const onTouchMove = (ev) => {
        if (isTouching()) {
          for (let ind = 0; ind < ev.targetTouches.length; ind++) {
            ...
            if (ev.targetTouches[ind].identifier === touch.touchIdentifier) {
                setTouch({
                    ...touch,
    ...
    const onTouchEnd = () =>{
        if (isTouching()) {
            ...
            if ((props.open && touchWidth < sidebarWidth - props.dragToggleDistance) ||
    ...
    const saveSidebarRef = (node) => {
      ...
      const hasBoxShadow =
        props.shadow && (touching || props.open || props.docked);
